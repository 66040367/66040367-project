...
import { ShoppingCart, Search, Star, Plus, Trash2, X } from 'lucide-react';
...
export default function Home() {
  ...
  // ระบบค้นหาสินค้า (ทำงานเรียลไทม์)
    ...
    <div className="min-h-screen bg-gray-100 font-sans">
      ...
      <header className="bg-orange-600 text-white sticky top-0 z-50 shadow-md">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <h1 className="text-2xl font-bold tracking-wider cursor-pointer" onClick={() => setSearchTerm('')}>
            ShopeeClone
          </h1>
          ...
          <div className="flex-1 max-w-xl relative">
            <input
              type="text"
              placeholder="ค้นหาสินค้า (เช่น หูฟัง, รองเท้า, นาฬิกา)..."
              className="w-full py-2 pl-4 pr-10 rounded-sm text-black focus:outline-none focus:ring-2 focus:ring-orange-300"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            {searchTerm ? (
              <X
                className="absolute right-3 top-2.5 text-gray-400 w-5 h-5 cursor-pointer hover:text-gray-600"
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <section className="md:col-span-2">
          <div className="flex justify-between items-center mb-4 border-b-2 border-orange-500 pb-2">
            <h2 className="text-xl font-bold text-gray-800">
              {searchTerm ? `ผลการค้นหาสำหรับ "${searchTerm}"` : 'สินค้าแนะนำประจำวัน'}
            </h2>
            <span className="text-sm text-gray-500">พบ {filteredProducts.length} รายการ</span>
          </div>
          ...
          {filteredProducts.length > 0 ? (
            ...
            <div className="bg-white p-8 text-center rounded-lg shadow">
              <p className="text-gray-500 text-lg">ไม่พบสินค้าที่ตรงกับคำค้นหา "{searchTerm}"</p>
              <button
                onClick={() => setSearchTerm('')}
                className="mt-4 text-orange-600 hover:underline text-sm font-semibold"
              >
                ดูสินค้าทั้งหมด
              </button>
            </div>
        ...
        <aside className="bg-white p-4 rounded-lg shadow h-fit sticky top-20">
          <h2 className="text-lg font-bold border-b pb-2 mb-4 text-gray-800">ตะกร้าสินค้าของคุณ</h2>
          ...
          {cart.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              <div className="pt-4 border-t">
                ...
                <button
                  onClick={() => alert('ทำรายการสั่งซื้อสำเร็จ!')}
